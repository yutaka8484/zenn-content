...
私は2022年の開業以来、基幹システムの入れ替えや生産管理の電子化を手掛けてきた。その中で、Docker＋ローカルLLMによる業務支援の環境をx1lite上に整えることはできた。しかし、**基幹システムから取得したデータを、Claude Codeや外部AIに安全に渡す仕組み**は意外と整わなかった。
...
Nangoを使ってClaude Codeに業務データを渡す流れを、公開ドキュメントをもとに簡単に説明する。
...
この構成で期待できる効果を、公開情報とドキュメントから整理すると次のとおりだ。
...
株式会社コバヤシでは、中小企業のDX推進・サーバー構築・業務システムのAPI設計を支援している。基幹システムとAIをどうつなぐか、具体案から相談に乗れる。
