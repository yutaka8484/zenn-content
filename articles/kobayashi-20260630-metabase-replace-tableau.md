...
私は**OSSのBIツール「Metabase」を自社サーバー上で運用し、Tableau依存を解消した**。本記事では、その導入経緯と具体的な運用方法を共有する。
...
私は「コバヤシWEBシステム」の開発・インフラ担当として、**Beelink EliteMini (x1lite) 上にUbuntu 24.04 + Docker環境を構築**している。全社のインフラ・業務システム・Webサイトをこの1台でホストしている。
...
Slack連携により、**品質不良率が閾値を超えた場合に担当者へ自動通知**される仕組みも構築した。これにより、問題発生から対応開始までのリードタイムを大幅に短縮できた。
...
コバヤシWEBシステムでは、**製造業・中小企業向けのDX・BI導入支援**を手がけている。TableauやPower BIからの移行、自社サーバー上でのOSS運用構築について、ぜひご相談いただきたい。
